...
namespace BG3.BagsOfSorting
{
    ...
    {
        private const string BASE_ICON = "Item_LOOT_Bag_Blackpowder";
        private const string SMALL_ICON_PREFIX = "SMALL_";
        private const string LARGE_ICON_PREFIX = "LARGE_";
        private const string SMALL_BASE_ICON = SMALL_ICON_PREFIX + BASE_ICON + ".png";
        private const string LARGE_BASE_ICON = LARGE_ICON_PREFIX + BASE_ICON + ".png";
...
";
        ...
        private static readonly string CONTENT_PATH = Path.Combine("Content");
        private static readonly string BAG_CONFIGURATION_FILE = Path.Combine(CONTENT_PATH, "Bags.json");
        private static readonly string CONTENT_STOCK_PATH = Path.Combine(CONTENT_PATH, "Stock");
        private static readonly string CONTENT_CUSTOM_PATH = Path.Combine(CONTENT_PATH, "Custom");
        private static readonly string ICONS_OUTPUT_PATH = Path.Combine("Output", "Icons");
        ...
        public static void Main(string[] args)
        {
            ...
            if (command == "--export-atlas-icons")
            {
                ExportAtlasIcons();
            }
            else if (command == "--generate-bags")
            {
                GenerateBags();
            }
            else if (command == "--add-bag")
            {
                AddBag();
            }
        }

        private static void ExportAtlasIcons()
        {
            var files = new DirectoryInfo(CONTENT_STOCK_PATH)
                .GetFiles("*.lsx", SearchOption.AllDirectories);

            Directory.CreateDirectory(ICONS_OUTPUT_PATH);
        ...
        private static void ExportAtlasIcons(string fileName)
        {
            ...
            foreach (var atlasIcon in atlas.Icons)
            {
                ...
                result.SaveAsPng(Path.Combine(ICONS_OUTPUT_PATH, $"{atlasIcon.MapKey}.png"));
            }
        }

        private static void GenerateBags()
        {
            using var bagsConfigurationFile = File.OpenRead(BAG_CONFIGURATION_FILE);
            ...
            File.Copy(BAG_CONFIGURATION_FILE, Path.Combine(BAGS_OUTPUT_PATH, "Bags.json"));
            ...
            {
                Console.WriteLine("All MapKeys have to be unique!");
        ...
        {
            bag.Name = $"{BAG_NAME}{bag.MapKey.ToString().Replace("-", string.Empty).ToLower()}";
        ...
        private static void GenerateIcon(BagConfiguration.Bag bag, bool alignGeneratedItemIconsRight, string outputPath)
        {
            ...
            else
            {
                var iconPath = bag.ItemIcon.Custom
                    ? Path.Combine(CONTENT_CUSTOM_PATH, $"{SMALL_ICON_PREFIX}{bag.ItemIcon.Name}.png")
                    : Path.Combine(ICONS_OUTPUT_PATH, $"{bag.ItemIcon.Name}.png");

                if (bag.ItemIcon.Generate)
                {
                    //TODO: Validate sizes and potentially resize for the user?
                ...
                {
                    //TODO: Resample file to make sure it's a PNG?
        ...
        private static void GenerateTooltipIcon(BagConfiguration.Bag bag, string outputPath)
        {
            ...
            else
            {
                var iconPath = bag.TooltipIcon.Custom
                    ? Path.Combine(CONTENT_CUSTOM_PATH, $"{LARGE_ICON_PREFIX}{bag.TooltipIcon.Name}.png")
                    : Path.Combine(ICONS_OUTPUT_PATH, $"{bag.TooltipIcon.Name}.png");
        ...
        private static void AddBag()
        {
            ...
            if (File.Exists(BAG_CONFIGURATION_FILE))
            {
                var bagsConfigurationFile = File.OpenRead(BAG_CONFIGURATION_FILE);
            ...
            bagsConfiguration.Bags.Add(new BagConfiguration.Bag
            {
                MapKey = Guid.NewGuid(),
            ...
            File.WriteAllText(BAG_CONFIGURATION_FILE, JsonSerializer.Serialize(bagsConfiguration, new JsonSerializerOptions
            {
                WriteIndented = true
            }));
        }
